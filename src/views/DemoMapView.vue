<template>
  <div class="relative w-full flex flex-col bg-[#0e162c] text-white font-sans overflow-hidden min-h-[100dvh]">
    ...
    <LoadingOverlay
      :open="loading"
      title="Demo Map"
      subtitle="Öffentliche Demo-Marker werden geladen…"
    />
    ...
    <div v-if="error" class="absolute inset-0 z-50 flex items-center justify-center p-6">
      <div class="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0b1228]/80 p-6 backdrop-blur-xl shadow-2xl">
        <h2 class="text-xl font-bold mb-2">Demo Map</h2>
...
// ✅ wichtig: in prod soll das auch gehen. Nimm /api als default, nicht localhost
...
async function loadDemoMarkers() {
  ...
  try {
    ...
    if (markers.value.length > 0) {
      // nice demo-start: Europa/Asien ungefähr -> oder nimm first marker
      const first = markers.value[0]
      mapRef.value?.flyTo(first.lat, first.lng, 3.8)
    }
  } catch (e) {
    console.error(e)
    error.value = 'Netzwerkfehler beim Laden der Demo Marker.'
    markers.value = []
  } finally {
    loading.value = false
  }
...
onMounted(async () => {
  ...
  // optional: center & zoom über query wie bei MapView/SharedView
