<!-- src/components/MarkerCreateModal.vue -->
<template>
  <Teleport to="body">
    <transition name="evoc-modal" appear>
    <div v-show="open" class="fixed inset-0 z-[80] flex items-center justify-center" @keydown.esc="close">
      <!-- Backdrop -->
  ...
  <div class="relative isolate w-[min(92vw,64rem)] mx-auto">
            ...
            <!-- Panel -->
            <div class="relative rounded-2xl bg-[#0b1228]/95 border border-white/1 ring-1 ring-white/1 shadow-2xl shadow-purple-900/50 overflow-hidden">
              <!-- Header -->
              <div class="flex items-center justify-between p-5 border-b border-white/10 bg-white/5">
                ...
                <button @click="close" class="p-2 rounded-lg hover:bg-white/10 text-gray-300 hover:text-white transition-colors">
                  ✕
                </button>
              </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div class="md:col-span-2">
                            ...
                            <div class="relative group mt-1">
                              <div class="pointer-events-none absolute -inset-[1px] rounded-xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition duration-300">
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400 via-fuchsia-300 to-indigo-400 blur-[8px]"></div>
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400/20 via-fuchsia-300/16 to-indigo-400/20"></div>
                          ...
                          <div class="md:col-span-2">
                            ...
                            <div class="relative group mt-1 isolate">
                              <div class="pointer-events-none absolute -inset-[1px] rounded-xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition duration-300 z-0">
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400 via-fuchsia-300 to-indigo-400 blur-[8px]"></div>
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400/20 via-fuchsia-300/16 to-indigo-400/20"></div>
                                <div class="absolute inset-[1px] rounded-[12px] bg-[#0e162c]"></div>
                              </div>
                      ...
                      <div class="rounded-2xl bg-white/5 border border-white/10 p-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <div class="relative group mt-1">
                              <div class="pointer-events-none absolute -inset-[1px] rounded-xl opacity-0 group-hover:opacity-100 transition duration-300">
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400 via-fuchsia-300 to-indigo-400 blur-[8px]"></div>
                                <div class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-purple-400/20 via-fuchsia-300/16 to-indigo-400/20"></div>
                                <div class="absolute inset-[1px] rounded-[12px] bg-[#0e162c]"></div>
                              </div>
                          ...
                          <!-- Auf Karte wählen (hier statt Footer) -->
                          <div class="col-span-2 pt-2">
                            <AppButton
                              variant="primary"
                              :to="`/mapview?goto=${encodeURIComponent(`${form.lat},${form.lng}`)}&zoom=12`"
                              class="h-12 md:h-12 px-4 md:px-5 shadow-lg shadow-fuchsia-900/20 hover:shadow-fuchsia-900/35 transition-shadow w-full"
                            >
...
<div
  ...
  <!-- dein bisheriger Inhalt der Dropzone muss in einen wrapper mit z-index -->
  <div class="relative z-10">
    <!-- ...hier bleibt alles, was vorher in der Dropzone drin war... -->
  </div>
    <!-- Hidden input -->
    <input
      ref="fileInputEl"
...
<div class="mt-5 border-t border-white/10 pt-4">
  ...
  <p class="mt-2 text-[11px] text-gray-400">
    Wähle, ob dieser Marker für Sharing sichtbar sein soll.
  </p>
</div>
                </form>
                <!-- ===== /Formular ===== -->
...
/* GeoSearch Events */
function onSelect(s: { lat: number; lon: number; display_name: string }) {
  form.value.lat = s.lat
  form.value.lng = s.lon
  if (!form.value.title?.trim()) {
    form.value.title = s.display_name.split(',')[0]
  }
}
...
async function uploadAll() {
  ...
  // nur hochgeladene Assets ins Form übernehmen
...
function onSubmit() {
  ...
  emit('submit', {
    ...
    placeName: form.value.placeName?.trim() || null,
  })
}
...
function toggleVisibility() {
  form.value.visibility = (form.value.visibility === 'PRIVATE' ? 'PUBLIC' : 'PRIVATE') as Visibility
}

function onSelectPlace(s: { lat:number; lon:number; display_name:string }) {
  form.value.lat = s.lat
  form.value.lng = s.lon
  form.value.placeName = primaryLabel(s.display_name) // ✅ Ort rein
}
...
function onSearch(payload: {
  ...
  if (payload.best) {
    form.value.lat = Number(payload.best.lat)
    form.value.lng = Number(payload.best.lon)

    const dn = payload.best.display_name ?? payload.query
    form.value.placeName = primaryLabel(dn) // ✅ HIER

    if (!form.value.title?.trim()) {
      form.value.title = primaryLabel(dn)
    }
  }
}
...
/* =========================
   Thumbnails (wenn du TransitionGroup nutzt)
   ========================= */
.thumb-enter-active,
.thumb-leave-active {
  transition:
